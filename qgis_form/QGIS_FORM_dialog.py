...
class QgisFORMDialog(QtWidgets.QDialog, FORM_CLASS):
    def __init__(self, parent=None):
        ...
        self.comboBox_phi.addItems(['Normal distribution', 'Lognormal distribution', 'Extvalue1 distribution', 'Exponential distribution', 'Gamma distribution', 'Weibull distribution'])
        self.comboBox_c.addItems(['Normal distribution', 'Lognormal distribution', 'Extvalue1 distribution', 'Exponential distribution', 'Gamma distribution', 'Weibull distribution'])
        self.comboBox_suw.addItems(['Normal distribution', 'Lognormal distribution', 'Extvalue1 distribution', 'Exponential distribution', 'Gamma distribution', 'Weibull distribution'])
    ...
    def HLRF_FORM_maerkang(self):
        ...
        def raster_array_SE(img_path, process, band_index=1):
            # 加载栅格图层
            ...
            # 获取指定波段的统计信息
            stats_ = rlayer.dataProvider().bandStatistics(band_index)
            ...
            # 读取指定波段的数据为数组
            arr = np.array(dataset_.GetRasterBand(band_index).ReadAsArray(0, 0, img_width_, img_height_), dtype=float)
        ...
        if self.checkBox.isChecked():
            phi_arr = raster_array_SE(img_path_phi1, 1, band_index=1)
        else:
            phi_arr = raster_array_SE(img_path_phi1, 0, band_index=1)
        phi_rad_arr = phi_arr * np.pi / 180

        # ----------------------phi_stat2------
        img_path_phi2 = self.mQgsFileWidget.filePath()
        if self.checkBox.isChecked():
            phi_stat2_arr = raster_array_SE(img_path_phi2, 1, band_index=2)
        else:
            phi_stat2_arr = raster_array_SE(img_path_phi2, 0, band_index=2)
        phi_stat2_rad_arr = phi_stat2_arr * np.pi / 180

        # ----------------------c_stat1------
        img_path_c1 = self.mQgsFileWidget_2.filePath()
        if self.checkBox_2.isChecked():
            c_arr = raster_array_SE(img_path_c1, 1, band_index=1)
        else:
            c_arr = raster_array_SE(img_path_c1, 0, band_index=1)

        # --------------------c_stat2----
        img_path_c2 = self.mQgsFileWidget_2.filePath()
        if self.checkBox_2.isChecked():
            c_arr_stat2 = raster_array_SE(img_path_c2, 1, band_index=2)
        else:
            c_arr_stat2 = raster_array_SE(img_path_c2, 0, band_index=2)

        # ----------------------suw_stat1---
        img_path_suw1 = self.mQgsFileWidget_3.filePath()
        if self.checkBox_3.isChecked():
            suw_arr = raster_array_SE(img_path_suw1, 1, band_index=1)
        else:
            suw_arr = raster_array_SE(img_path_suw1, 0, band_index=1)

        # -----------------------suw_stat2---
        img_path_suw2 = self.mQgsFileWidget_3.filePath()
        if self.checkBox_3.isChecked():
            suw_arr_stat2 = raster_array_SE(img_path_suw2, 1, band_index=2)
        else:
            suw_arr_stat2 = raster_array_SE(img_path_suw2, 0, band_index=2)
        ...
        if current_text_phi == 'Normal distribution':
            a_phi = 1
        elif current_text_phi == 'Lognormal distribution':
            a_phi = 2
        elif current_text_phi == 'Extvalue1 distribution':
            a_phi = 3
        elif current_text_phi == 'Exponential distribution':
            a_phi = 4
        elif current_text_phi == 'Gamma distribution':
            a_phi = 6
        elif current_text_phi == 'Weibull distribution':
            a_phi = 7

        if current_text_c == 'Normal distribution':
            a_c = 1
        elif current_text_c == 'Lognormal distribution':
            a_c = 2
        elif current_text_c == 'Extvalue1 distribution':
            a_c = 3
        elif current_text_c == 'Exponential distribution':
            a_c = 4
        elif current_text_c == 'Gamma distribution':
            a_c = 6
        elif current_text_c == 'Weibull distribution':
            a_c = 7

        if current_text_suw == 'Normal distribution':
            a_suw = 1
        elif current_text_suw == 'Lognormal distribution':
            a_suw = 2
        elif current_text_suw == 'Extvalue1 distribution':
            a_suw = 3
        elif current_text_suw == 'Exponential distribution':
            a_suw = 4
        elif current_text_suw == 'Gamma distribution':
            a_suw = 6
        elif current_text_suw == 'Weibull distribution':
            a_suw = 7
